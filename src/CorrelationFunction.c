...
double CorFunWitMathole(double lag, double scale, double smooth,double R_power1)
{
 ...
 /*first version    with onef2      
  ...
  rho=a1+(K1/K2)*a2;*/
   double a1,a2,a3,a4,res=0.0;
   int n;
 for(n=0;n<=k;n++){
    a1=1/(gammafn(n+1)*gammafn(k-n+1));
    a2=bessel_i(x,n-smooth,1)/gammafn(d/2+n);
    a3=(bessel_i(x,n+smooth,1)*gammafn(smooth+d/2+k))/(gammafn(d/2+k)*gammafn(smooth+d/2+n));
    a4=R_pow(0.5*x,n+0.5);
    res=res+a1*a4*(a2-a3);
}
  rho= (gammafn(k+1)*gammafn(d/2)*gamma(0.5))* res;
   return(rho);
 }
