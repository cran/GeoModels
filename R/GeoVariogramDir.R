GeoVariogramDir <- function(data, coordx, coordy = NULL, coordz = NULL,
  ...
  if (is.null(neighb)) {
    if (is.finite(maxdist)) {
      # Estimate expected neighbors within maxdist based on spatial density
      if (ncol(coords) == 2) {
        ...
        # Use 2x expected to ensure good coverage
        neighb <- min(n - 1, max(100, as.integer(expected_neighb * 2)))
      } else {
        # 3D case: use volume-based estimate
      ...
      # Without maxdist: use ALL neighbors (chunking will handle memory)
      neighb <- n - 1
    }
  }
  
  # Estimate memory and determine chunk size
  # Each pair needs ~24 bytes (3 doubles: dist, angle component, angle component)
  memory_estimate_mb <- (n * neighb * 24) / 1e6
  n_pairs_total <- n * neighb
  
  # Adaptive chunking based on neighb size
  if (neighb < 500) {
    # Small neighb: can process in larger chunks
    chunk_size <- n  # No chunking needed
  } else if (neighb < 5000) {
    # Medium neighb: moderate chunks (~1GB target)
    chunk_size <- max(500, min(n, as.integer(1000e6 / (neighb * 24))))
  } else {
    # Large neighb: small chunks (~500MB target)
    chunk_size <- max(100, min(n, as.integer(500e6 / (neighb * 24))))
  }
  ...
  if (is.finite(maxdist)) {
    ...
    # Estimate from sample if n is large
    if (n > 10000) {
      samp <- sample(n, min(1000, n))
      dists_samp <- as.matrix(dist(coords[samp, ]))
      maxdist_bin <- quantile(dists_samp[upper.tri(dists_samp)], 0.95)
    } else {
...
#  numvario <- numbins
#  mm <- c(breaks[1], tail(breaks, 1))


breaks   <- seq(0, maxdist_bin, length.out = numbins + 1)
widths   <- diff(breaks)                     # larghezze dei bin
centers  <- breaks[-1] - widths/2            # centri dei bin
numvario <- numbins                          # <— serve più avanti
mm       <- c(breaks[1L], breaks[length(breaks)])  # oppure: range(breaks)
  
  # Precompute directional angle ranges
  ...
  # Helper: geodesic distance functions
  use_geo <- distance %in% c("Geo", "Chor")
  
  proj_sinusoidal <- function(M) {
    # Project lat/lon to sinusoidal for KNN search
    lon <- M[,1] * pi / 180
    lat <- M[,2] * pi / 180
    cbind(lon * cos(lat), lat)
  }
  
  chord_from_arc <- function(theta) {
    2 * sin(theta / 2)
  }
  
  # Prepare coordinates for KNN
  if (use_geo) {
    coords_knn <- proj_sinusoidal(coords)
  } else {
    coords_knn <- coords
  }
 ...
 # if (n_chunks > 1) {
 #   message(sprintf("Processing %d points in %d chunks (neighb=%d, ~%.0f MB per chunk)", 
 #                   n, n_chunks, neighb, (chunk_size * neighb * 24) / 1e6))
 # }
  ...
  for (chunk_id in seq_len(n_chunks)) {
    ...
    # KNN search for this chunk
    query_coords <- coords_knn[chunk_indices, , drop = FALSE]
    
    if (is.finite(maxdist)) {
      # With radius constraint
      if (use_geo) {
        # Approximate radius in projected space (conservative)
        radius_proj <- maxdist * 1.1  # slight overestimate for safety
    ...
    # Filter valid pairs and apply maxdist constraint
    keep <- idx_mat > 0
    if (is.finite(maxdist)) {
      keep <- keep & (dist_mat <= maxdist)
    }
    
    if (!any(keep)) next
    
    # Flatten to pair lists
    ...
    # Auto-select binning method: use C if > 5000 pairs in this chunk
    n_pairs_chunk <- length(i)
    use_c_chunk <- n_pairs_chunk > 5000
    
    # Accumulate for each direction
    if (use_c_chunk) {
      # C binning (faster for large chunks)
      for (k in seq_along(directions)) {
        mask <- mask_dir_mat[,k]
        if (any(mask)) {
      ...
      # R binning (simpler for small chunks)
